...
* FIX: "Manage Themes" items list is broken when anchor links are present in the displayed description  
* FIX: Convert to InnoDB table not working under the WP-Optimize module's "Table Info" area
* FIX: Select/deselect buttons not working when choosing WordPress database tables for backup
* FIX: UpdraftCentral unresponsive after closing the "test connection methods" dialog
* TWEAK: Add WP-Optimize action button (tab) images
* TWEAK: Add styles for the latest changes applied to the WP-Optimize module
* TWEAK: Fix WP_Theme_JSON_Resolver::theme_has_support deprecation error 
* TWEAK: Fix deprecated notices for sites_table and sitemeta_table
* TWEAK: Fix dropdown menu's transparency and alignment
* TWEAK: Replace the default Datatable sorting symbols with HTML entities to prevent unfamiliar sorting arrows caused by different charsets
* TWEAK: Suppress some PHP 8.2 deprecation notices from use of ${} style variables
* TWEAK: The installation modal dialog of plugin/theme now has a new look where the option to activate or not to activate the plugin/theme is transformed to a checkbox
* TWEAK: To stay the user remains on the dashboard, open the documentation page in a new tab. 
* TWEAK: Update phpseclib library from version 1 to 2.
* TWEAK: Use UpdraftCentral version when loading bootstrap style
* TWEAK: Fix popup window close button UI.
* TWEAK: Include WP_Screen class while enqueuing editor scripts. This is to ensure that the WordPress class which is by default included for the admin dashboard is also included for the UpdraftCentral dashboard as well thus preventing fatal error from happening especially when the class is instantiated by other plugins during UpdraftCentral page load.
* TWEAK: Resolve accessibility issue.
* TWEAK: Add the ability for user to add the UpdraftCentral dashboard as a Gutenberg block
* TWEAK: Add updates filter/option to show/hide incompatible plugins/themes for update
* TWEAK: Translation items are added in the search result when the "Patch releases" checkbox is checked
* TWEAK: Split multiple sentences into separate translation calls

= 0.8.29 - 23/May/2023 =

* FIX: Don't check what file type of chunks that are sent from the browser when manually uploading a plugin/theme in chunks because it can lead to a false positive result
* FIX: "Show updates for all updates" hangs and never completes when UTF8MB4 characters and/or double and single quotes are present on the site title.
* TWEAK: Create abstraction layer for access to UpdraftCentral tables
* TWEAK: Show the install and/or activate option when UpdraftPlus plugin is not active on the remote site and the backup feature is accessed outside the "Backups" area. 
* TWEAK: Disable backup feature if UpdraftPlus is currently not active on the remote website

= 0.8.28 - 23/Dec/2022 =

* FIX: Remove double-escaping leading to printing of HTML in site-row template

= 0.8.27 - 20/Dec/2022 =

* TWEAK: Tweak query for site tags retrieval
* TWEAK: Tweak query for setting and retrieving of user metas
* FIX: Fix broken horizontal notice

= 0.8.26 - 17/Dec/2022 =

* FIX: Fix broken hidden menus
* TWEAK: Add pCloud icon for a better look on the remote storage button selector and reduce the size of pCloud logo on the configuration template
* TWEAK: Trasfer "refresh_available_updates_display" method from updates module to UpdraftCentral class for reuse

= 0.8.25 - 10/Dec/2022 =

* TWEAK: Upgrade bootstrap to 5.2.3 (latest)
* TWEAK: Upgrade bootbox to 6.0.0 (latest)
* TWEAK: Remove no longer needed libraries (e.g. momentjs and popperjs)
* TWEAK: Adjust UI elements (e.g. dropdowns, badges, modal, etc.) based on latest bootstrap changes
* TWEAK: Adjust modal dialog initialization code

= 0.8.24 - 01/Dec/2022 =

* FIX: Remove unused code introduced in 0.8.23 which opened up a cross-site request HTTP forgery hole.

= 0.8.23 - 28/Nov/2022 =

* FIX: Plugin and Theme modules causes an issue when php_events are exposed/returned from the remote website using PHP 8.1
* FIX: Plugin and Theme modules freezes when bulk actions are executed
* TWEAK: Ensure compatibility of the Handlebars partial templates of UpdraftPlus
* TWEAK: Display appropriate plugin and theme failure message
* TWEAK: Load events initially based on latest entries 
* TWEAK: Add website filter to events table
* TWEAK: Improve SQL statement when retrieving captured events
* TWEAK: Prevent UpdraftVault to display multiple error dialogs when UpdraftPlus plugin is not installed or active on the remote site
* TWEAK: Add IndexedDB Storage as the default storage for getting and saving metadata
* TWEAK: Change "UpdraftCentral.get_wporg_metadata" to Promise method but preserved legacy call for compatibility
* TWEAK: Remove pre-filling of metadata when loading items for updates
* TWEAK: Add on-demand request to WordPress.org when data is not available or has expired
* TWEAK: Align the new site confirmation dialog on the center
* TWEAK: Upgrade block editor with the latest changes applied to WordPress core

= 0.8.22 - 09/Jul/2022 =

* FIX: Non-functional manual authentication button
* FIX: Default label empty when adding additional storage account using UpdraftPlus premium
* FIX: Unexpected in-progress dialog display when triggering a button with two or more selectors
* FIX: Prevent updates process to stop due to invalid cached data
* FIX: The instance label/title of the remote storage was unclickable, hence the editing of the label could only be done through the UpdraftPlus plugin 
* FIX: The 'Only email the database backup' checkbox of the UpdraftPlus' reporting settings is not clickable
* TWEAK: Add a CSS class that can be used across all remote storage to prevent UpdraftPlus' double warning being displayed on the pre-configuration form/template
* TWEAK: Add new CSS classes (pre-defined width in pixel size) as a helper to harmonise the size of the HTML input tags on each remote storage
* TWEAK: Update class-udrpc
* TWEAK: Trigger an UpdraftCentral event on error
* TWEAK: Update notice class

= 0.8.21 - 11/May/2022 =

* FIX: Invalidate entities with the same post ID when editing post or page using the block editor
* FIX: Email remote storage option when pressed displays blank or empty content
* TWEAK: Improve UpdraftPlus module in presenting FTP Handlebars template that is now CSP-compliant
* TWEAK: Loading and processing updates in parallel
* TWEAK: Remember or persist the backup first option selected by the user from previous updates process
* TWEAK: Initialize an empty array if command currently does not have entries in local cache collection
* TWEAK: Limit search on site title and url
* TWEAK: Limit updates on the mass updates area to sites filtered by the user using tags
* TWEAK: Pass UpdraftCentral_User instance as parameter for "updraftcentral_get_user_tags" filter
